...
class FedSGDClient(object):
    ...
    def update_model(self, weights):
        self.trainer.client_trainer.set_model_params(weights)

    def update_dataset(self, client_index):
        self.client_index = client_index
        self.train_local = [self.train_data_local_dict[id] for id in client_index]
        logging.info(f"train_dl is None: {self.train_local is None}, len: {len(self.train_local) if self.train_local is not None else 'None'}")
    ...
    def train_with_data_id(self, round_idx = None, data_id = 0):
        ...
        # weights = self.trainer.get_model_params()
        weights = [para.detach().cpu() for para in self.trainer.client_trainer.grad]

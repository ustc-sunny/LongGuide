...
if __name__ == "__main__":
    ...
    model_config, training_model, tokenizer = create_model(
        model_args, formulation="classification")

    # trainer
    if args.fl_algorithm == "FedFwd":
        client_trainer = ForwardTextClassificationTrainer(
            model_args, device, copy.deepcopy(training_model), None, None)
        cloud_trainer = ForwardTextClassificationTrainer(
            model_args, device, copy.deepcopy(training_model), None, None)
        server_trainer = ForwardTextClassificationTrainer(
            model_args, device, copy.deepcopy(training_model), None, None)
    ...
    if process_id == 1:
        server_trainer.test_dl = test_data_global

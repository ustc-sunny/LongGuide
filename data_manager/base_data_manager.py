...
class BaseDataManager(ABC):
    ...
    def get_last_n_clients_indices(self, partition_file, partition_method, n):
        """
        合并分区文件中最后 n 个 client 的 train/test 索引
        返回：train_index_list, test_index_list
        """
        total_clients = self.args.client_num_in_total
        start_id = total_clients - n
        end_id = total_clients
        train_index_list = []
        test_index_list = []
        for client_id in range(start_id, end_id):
            train_index_list.extend(partition_file[partition_method]["partition_data"][str(client_id)]["train"][()])
            test_index_list.extend(partition_file[partition_method]["partition_data"][str(client_id)]["test"][()])
    ...
    def _load_federated_data_cloud(self):
        ...
        if state:
            ...
            # 合并后200个client的train/test索引
            if self.args.dataset == "agnews":
                n_adaptive = 100
            elif self.args.dataset == "yahoo":
                n_adaptive = 50
            elif self.args.dataset == "yelp-p":
                n_adaptive = 22
            else:
                n_adaptive = 100
            train_index_list, test_index_list = self.get_last_n_clients_indices(partition_file, partition_method, n_adaptive)#(n=100)

            # # 合并后200个client的train/test索引
            
            # train_index_list, test_index_list = self.get_last_n_clients_indices(partition_file, partition_method, n=100)
